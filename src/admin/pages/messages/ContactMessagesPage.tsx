    ...
    () => [
      ...
      { key: 'subject', header: 'Subject' },
      {
        ...
        render: (row: MessageItem) => (
          <select
            className="rounded border border-white/10 bg-[#0b132000]/60 px-2 py-1 text-xs text-[#C9D1D9]"
      ...
      },
      {
        ...
        header: 'Detail',
        render: (row: MessageItem) => (
          <Link className="text-[#06B6D4] underline" to={`/mhe-control-center/messages/${row.id}`}>
            View
          </Link>
        ),
      },
  ...
  return (
    <div className="space-y-6">
      ...
      {rows.length > 0 ? (
        ...
        <div className="rounded-xl border border-white/10 bg-[#0b132000]/80 p-12 shadow-sm">
          <div className="flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 rounded-full bg-[#06B6D4]/10 flex items-center justify-center mb-4">
              <Inbox className="w-8 h-8 text-[#06B6D4]" />
            </div>
