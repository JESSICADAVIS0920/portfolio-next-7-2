...
export function DataTable<T>({ columns, rows, rowKey, onEdit, onDelete, renderMobileItem }: DataTableProps<T>) {
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-[#0b132000]/80 shadow-sm">
      {/* Mobile View */}
      <div className="md:hidden p-4 space-y-4">
        ...
        {rows.map((row) => (
          <div key={rowKey(row)} className="bg-[#0b132000] border border-white/10 rounded-xl p-4 space-y-3">
            {renderMobileItem ? (
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="bg-gradient-to-r from-[#0b132000]/60 to-[#0b132000]/80 border-b border-white/10">
            <tr>
              {columns.map((col) => (
                ...
                {(onEdit || onDelete) && (
                  <td className="px-5 py-4">
                    <div className="flex gap-2">
                      {onEdit && (
                        <button
                          className="inline-flex items-center gap-1.5 rounded-lg bg-[#06B6D4]/10 border border-[#06B6D4]/30 px-3 py-1.5 text-xs font-medium text-[#06B6D4] hover:bg-[#06B6D4]/20 transition-colors"
                          onClick={() => onEdit(row)}
                        >
                          <Edit2 className="w-3.5 h-3.5" />
                          Edit
