...
export function AdminLayout() {
  ...
  const activeLink = 'text-white bg-gradient-to-r from-[#06B6D4]/20 to-[#0891B2]/20 border border-[#06B6D4]/30';
      ...
      `}>
        ...
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-4">
          <div className={`flex items-center gap-3 ${isCollapsed ? 'hidden' : ''}`}>
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#06B6D4] to-[#0891B2] shadow-lg shadow-[#06B6D4]/30">
              <LayoutDashboard className="h-5 w-5 text-white" />
            </div>
            <div>
              <div className="text-sm font-bold text-white">Admin Panel</div>
              <div className="text-xs text-[#06B6D4]">Portfolio CMS</div>
            </div>
          </div>
          <button
            type="button"
            className="hidden lg:flex h-9 w-9 items-center justify-center rounded-xl bg-[#0B1320]/60 border border-white/10 text-white/60 hover:bg-white/5 hover:text-white hover:border-[#06B6D4]/50 transition-all"
          ...
          <button
            type="button"
            className="lg:hidden flex h-9 w-9 items-center justify-center rounded-xl bg-white/10 text-white/60 hover:bg-[#06B6D4]/20 hover:text-white transition-colors"
            onClick={() => setIsMobileSidebarOpen(false)}
        ...
        <nav className="h-[calc(100%-5rem)] overflow-y-auto px-3 py-6 space-y-6">
          {navSections.map((section) => (
            <div key={section.title}>
              {!isCollapsed && (
                <div className="px-3 mb-3 text-xs font-semibold uppercase tracking-wider text-[#06B6D4]">
                  {section.title}
                </div>
              )}
        ...
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-white/10 bg-[#0B1320]/90 backdrop-blur-xl px-6">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              ...
              <input
                className="w-64 md:w-80 rounded-xl border border-white/10 bg-[#0B1320]/60 py-2.5 pl-11 pr-4 text-sm text-white placeholder-white/60 focus:border-[#06B6D4] focus:bg-white/5 focus:outline-none focus:ring-4 focus:ring-[#06B6D4]/20 transition-all"
                      ...
                      <li key={`${result.collection}-${result.id}`}>
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <div className="text-xs text-[#06B6D4] uppercase">{result.collection}</div>
                          </div>
                        </NavLink>
                      </li>
                    ))}
              ...
              >
                {data.singletons.about?.profileImageUrl ? (
                  <img
                    ...
                    alt="Admin avatar"
                    className="h-8 w-8 rounded-lg border-2 border-[#06B6D4]/30 object-cover"
                  />
                ) : (
                  <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-[#06B6D4] to-[#0891B2] text-sm font-bold text-[#0B1320]">
              ...
              {isMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-xl border border-white/10 bg-[#0B1320]/95 shadow-xl shadow-black/30 overflow-hidden animate-scale-in backdrop-blur-xl">
                  ...
                  <div className="py-2">
                    ...
                    <button
                      type="button"
                      className="flex w-full items-center gap-3 px-4 py-2.5 text-sm text-[#06B6D4] hover:bg-[#06B6D4]/10 transition-colors"
                      onClick={async () => {
                        await signOut();
