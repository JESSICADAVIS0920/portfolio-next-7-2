import { Mail, Phone, MapPin, ArrowUpRight, Heart, Github, MessageCircle } from 'lucide-react';
  ...
  return (
    <footer className="relative w-full bg-[#0B1320] text-white overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-[#06B6D4] rounded-full opacity-[0.08] blur-[100px] animate-nebula-1" />
        <div className="absolute -bottom-40 -left-40 w-[500px] h-[500px] bg-[#0891B2] rounded-full opacity-[0.06] blur-[120px] animate-nebula-2" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#06B6D4] rounded-full opacity-[0.03] blur-[150px]" />
      </div>
      ...
      <div className="relative mx-auto w-full max-w-[1400px] px-6 sm:px-8 lg:px-12 xl:px-16 pt-16 sm:pt-20 pb-8 sm:pb-10">
        ...
        <div className="grid gap-8 sm:gap-12 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mb-12 sm:mb-16">
            ...
            <div className="flex items-center gap-3 group">
              {about.profileImageUrl ? (
                <img
                  ...
                  className="h-14 w-14 rounded-2xl border-2 border-white/10 object-cover group-hover:scale-105 group-hover:border-[#06B6D4]/50 transition-all duration-300"
                />
              ) : (
                <div className="h-14 w-14 rounded-2xl bg-gradient-to-br from-[#06B6D4] to-[#0891B2] flex items-center justify-center text-xl font-bold group-hover:scale-105 group-hover:rotate-3 transition-all duration-300 shadow-lg shadow-[#06B6D4]/20">
                  {(about.fullName ?? 'P').charAt(0)}
                </div>
              )}
              <div>
                <h2 className="text-xl font-bold text-white group-hover:text-[#06B6D4] transition-colors">{about.fullName ?? 'Portfolio'}</h2>
          ...
          <nav className="space-y-6 animate-fade-in" style={{ animationDelay: '100ms' }} aria-label="Footer navigation">
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#06B6D4]">Quick Links</h3>
              ...
              {quickLinks.map((link, index) => (
                <li key={link.label} style={{ animationDelay: `${index * 50}ms` }}>
                  {link.isAnchor ? (
                    ...
                    >
                      <span className="w-1.5 h-1.5 rounded-full bg-[#06B6D4] opacity-0 group-hover:opacity-100 transition-opacity" />
                      {link.label}
                    </button>
                  ) : (
                    <Link 
          ...
          <address className="not-italic space-y-6 animate-fade-in" style={{ animationDelay: '200ms' }}>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#06B6D4]">Contact</h3>
            <div className="space-y-4">
              {contactInfo.email && (
                <a href={`mailto:${contactInfo.email}`} className="flex items-center gap-3 text-white/60 hover:text-white hover:translate-x-1 transition-all duration-300 group">
                  <div className="w-10 h-10 rounded-xl bg-[#0B1320]/60 border border-white/10 flex items-center justify-center group-hover:bg-[#06B6D4] group-hover:border-[#06B6D4] group-hover:scale-110 transition-all duration-300">
                    <Mail size={16} />
                  </div>
          ...
          <section className="space-y-6 animate-fade-in" style={{ animationDelay: '300ms' }}>
            <h3 className="text-sm font-semibold uppercase tracking-wider text-[#06B6D4]">Let's Work Together</h3>
            <p className="text-sm text-white/60">
              Have a project in mind? Let's create something amazing together.
            </p>
            <Link
              to="/contact"
              className="group inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-[#06B6D4] text-[#111827] font-semibold hover:shadow-xl hover:shadow-[#06B6D4]/30 hover:-translate-y-1 hover:scale-105 transition-all duration-300"
            >
              Get in Touch
